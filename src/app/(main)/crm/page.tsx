...
import { Users, Building2, Plus, Search, Phone, Mail, ChevronRight, ListTodo } from 'lucide-react'
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="min-w-0">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">{company.name}</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{company.industry || '업종 미입력'}</p>
        </div>
        <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0">{contactCount}명</span>
      </div>
        ...
        loadingCompanies ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredCompanies.map(c => (
              <CompanyCard key={c.id} company={c} contactCount={contactCountByCompany[c.id] ?? 0} />
            ))}
          </div>
        )
      )}
